...
$(function() {
        $('#reg_btn').click(
            function () {
                ...
                $.ajax({
                        url: 'do-register',
                        type: 'post',
                        data: {'username': username, 'password': password,'repassword': repassword,'name':name,'mobile':mobile,'address':address,'csrfmiddlewaretoken':csrf},
                        dataType: 'json',
                        success: function (data) {
                            if (data.msg == 'account_error') {
                                $('#error_msg').text('账号已存在')
                            } else if (data['msg'] == 'passwd_error') {
                                $('#error_msg').text('密码输入不一致')
 ...
 // 登录时判断页面内容是否输入正确
 $(function () {
     var div_1 = "<div id='error_msg'></div>"
     // 在指定元素后添加元素
     $('#toRegister').after(div_1);
     $('#login_btn').click(
         function () {
             csrf = $('input').val()
             ...
             $.ajax({
                 type: 'post',
                 url: '/do-login/',
                 data: {'csrfmiddlewaretoken':csrf,'username':username,'password':password,'veri':veri},
                 dataType: 'json',
                 success: function(data){
                     $('#error_msg').css('color','red')
                     if (data['msg'] == 'eroor_veri'){
                            $('#error_msg').text('验证码错误')
                     }
                     else if (data['msg'] == 'eroor_password'){
                            $('#error_msg').text('密码错误')
                     }
                     else if (data['msg'] == 'no_account'){
                            $('#error_msg').text('用户不存在或不可用，请先注册')
                     }
                     else if (data['msg'] == 'success'){
                            location.href = '/index/'
                     }
